...
export default function WorkspaceViewPage() {
  ...
  const sidebarContent = (
    <>
      {/* Workspace header */}
      <div className="px-4 pt-4 pb-3" style={{ borderBottom: '1px solid var(--color-ce-border)' }}>
        <button
        ...
        <div className="flex items-center gap-3">
          <div className={`w-9 h-9 rounded-lg avatar-color-${avatarIndex(currentWorkspace?.name)} flex items-center justify-center text-white text-sm font-bold shadow-sm flex-shrink-0`}>
          ...
          <div className="min-w-0 flex-1">
            ...
            <button
              onClick={() => { setShowMembers(true); setSidebarOpen(false); }}
          ...
          {channels.map((ch) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-[7px] rounded-lg text-[13px] font-medium transition-all duration-150 cursor-pointer relative flex items-center justify-between group disabled:opacity-50 disabled:cursor-wait ${
                  isActive
                    ? "channel-active text-ce-text-primary"
                    : ch.isMember
                    ? "text-ce-text-secondary hover:text-ce-text-primary hover:bg-ce-bg-hover"
                    : "text-ce-text-muted hover:text-ce-text-tertiary hover:bg-ce-bg-hover"
                }`}
              ...
              >
                ...
                {!ch.isMember && (
                  <span className="text-[10px] font-medium opacity-0 group-hover:opacity-100 transition-opacity"
                        style={{ color: 'var(--color-ce-accent)' }}>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-0.5">
              {onlineUsers.map((u) => (
                <div
                  key={u.id}
                  className="flex items-center gap-2.5 px-3 py-[6px] rounded-lg text-[13px] text-ce-text-secondary hover:text-ce-text-primary hover:bg-ce-bg-hover transition-colors"
      ...
      <div className="px-3 py-3 flex items-center justify-between"
        ...
        <button
          onClick={logout}
          className="p-2 rounded-lg text-ce-text-muted hover:text-ce-text-primary hover:bg-ce-bg-hover transition-all cursor-pointer flex-shrink-0"
          title="Sign out"
        >
  ...
  return (
    <div className="h-screen flex" style={{ background: 'var(--color-ce-bg-primary)' }}>
      {/* ====== Desktop Sidebar ====== */}
      <aside className="hidden md:flex w-[272px] flex-shrink-0 flex-col animate-slide-in-left"
             style={{ background: 'var(--color-ce-bg-secondary)', borderRight: '1px solid var(--color-ce-border)' }}>
        {sidebarContent}
      </aside>
      ...
      {sidebarOpen && (
        <>
          ...
          <aside className="sidebar-drawer md:hidden flex flex-col"
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="md:hidden flex items-center gap-3 px-4 py-3 flex-shrink-0"
          ...
          <button
            ...
            className="p-2 -ml-2 rounded-lg text-ce-text-secondary hover:bg-ce-bg-hover transition-colors cursor-pointer"
          ...
          <div className="flex items-center gap-2 min-w-0">
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-ce-text-muted flex-shrink-0">
              <path d="M4 9h16" /><path d="M4 15h16" /><path d="M10 3L8 21" /><path d="M16 3l-2 18" />
            </svg>
            <span className="text-sm font-semibold text-ce-text-primary truncate">
              {currentChannel?.name || currentWorkspace?.name || "CollabEngine"}
            </span>
          </div>
        </div>
